...
def waterJug(jug1, jug2, result):
	...
	while (len(jugQueue) > 0):
		...
		if (jugCurrent[0] == result or jugCurrent[1] == result):
			isTargetReach = True
			
			if (jugCurrent[0] == result):
				if (jugCurrent[1] != 0):
		...
		jugQueue.append([jugCurrent[0], jug2]) # Fill Jug2
		jugQueue.append([jug1, jugCurrent[1]]) # Fill Jug1
		...
		jugQueue.append([jug1, 0])
		
		
		jugQueue.append([0, jug2])
